...
const HorodatageStatusView = ({ setSnackbar, isMobile = false, isTablet = false }) => {
    ...
    const fetchStatus = async () => {
        setLoading(true);
        try {
            const response = await API.get('/horodatage/statut');
            setStatus(response.data);
        } catch (error) {
            console.error("Erreur:", error);
            if (setSnackbar) setSnackbar({ open: true, message: "Erreur lors du chargement du statut", severity: 'error' });
    ...
    const testerHorodatage = async () => {
        setTesting(true);
        try {
            const response = await API.post('/horodatage/tester', { data: testData || "Test automatique" });
            setTestResult(response.data);
            if (setSnackbar) setSnackbar({ open: true, message: response.data.success ? "Test réussi ✅" : "Test échoué ❌", severity: response.data.success ? 'success' : 'error' });
        } catch (error) {
            setTestResult({ success: false, message: error.message });
            if (setSnackbar) setSnackbar({ open: true, message: "Erreur lors du test", severity: 'error' });
    ...
    return (
        <Box sx={{ px: { xs: 0, sm: 1 } }}>
            <Grid container spacing={mobile ? 2 : 3}>
                    ...
                    <Paper sx={{ p: mobile ? 2 : 3 }}>
                        ...
                        <Stack spacing={2}>
                            ...
                            <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 1 }}>
                                ...
                                <Typography variant="body2" sx={{ fontFamily: 'monospace', fontSize: mobile ? '0.7rem' : '0.8rem' }}>
                                    {status?.tsaUrl}
                                </Typography>
                            </Box>
                            <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 1 }}>
                        ...
                        <Button variant="outlined" startIcon={<RefreshIcon />} onClick={fetchStatus} sx={{ mt: 3 }} fullWidth>
                            Actualiser
                        </Button>
                    </Paper>
                </Grid>
                ...
                <Grid item xs={12} md={6}>
                    <Paper sx={{ p: mobile ? 2 : 3 }}>
                        <Typography variant={mobile ? "subtitle1" : "h6"} gutterBottom>
                            <VerifiedIcon sx={{ mr: 1, verticalAlign: 'middle' }} />Test du service
                        </Typography>
                        <Divider sx={{ my: 2 }} />
                        <Stack spacing={2}>
                            <TextField 
                                label="Données à horodater" 
                                value={testData} 
                            ...
                            {testResult && (
                                <Alert severity={testResult.success ? "success" : "error"} sx={{ mt: 2 }}>
                                    <Typography variant="body2" fontWeight="bold">{testResult.message}</Typography>
                                    {testResult.valide !== undefined && 
                                        <Typography variant="caption">Token valide : {testResult.valide ? "✅ Oui" : "❌ Non"}</Typography>
                                    }
                                </Alert>
                            )}
                        </Stack>
                ...
                <Grid item xs={12}>
                    <Paper sx={{ p: mobile ? 2 : 3 }}>
                        ...
                        <Divider sx={{ my: 2 }} />
                        <Typography variant="body2" color="textSecondary" paragraph sx={{ fontSize: mobile ? '0.75rem' : '0.875rem' }}>
